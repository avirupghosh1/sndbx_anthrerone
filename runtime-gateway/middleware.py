...
from control_plane import SandboxRoute, SandboxRouteFailure, get_control_plane_client
...
def _layer2_deny(
    ...
    *,
    ...
    if not expected:
        return None
    header_name = (
        getattr(cfg, "INGRESS_SHARED_TOKEN_HEADER", None) or "X-Ingress-Access-Token"
    ).strip()
    if not header_name:
        return None
    provided = (headers.get(header_name.lower()) or "").strip()
    if not provided:
        return 401, f"missing {header_name}"
    if not hmac.compare_digest(provided, expected):
        return 401, f"invalid {header_name}"
    return None


def host_is_local_route(host_header: str, scope: Scope) -> bool:
    return _parse_local_query_route(scope, host_header) is not None
