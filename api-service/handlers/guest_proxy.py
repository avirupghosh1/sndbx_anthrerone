...
from handlers.guest_proxy_utils import forward_headers as build_forward_headers, gateway_ws_url
...
def _provided_traffic_token(websocket: WebSocket) -> Optional[str]:
    token = (
        (websocket.headers.get("e2b-traffic-access-token") or "").strip()
        or (websocket.query_params.get("traffic_token") or "").strip()
        or (websocket.query_params.get("traffic_access_token") or "").strip()
...
async def _traffic_token_for_proxy(
    ...
    *,
    ...
    token = _provided_traffic_token(websocket)
    if token:
        return token
    if not allow_mint:
        return None
