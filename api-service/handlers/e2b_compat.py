...
def _dockerfile_from_e2b_template_payload(body: dict[str, Any]) -> tuple[str, dict[str, str], str, str]:
    ...
    for step in body.get("steps") or []:
        ...
        if typ == "COPY":
            raise HTTPException(
                status_code=400,
                detail="E2B SDK COPY/file-upload template builds are not implemented yet.",
            )
        if typ == "ENV":
    ...
    return "\n".join(lines) + "\n", env, str(body.get("startCmd") or ""), str(body.get("readyCmd") or "")
...
async def start_template_build(
    ...
    body = await _json_body(request)
    try:
        dockerfile, env, start_cmd, ready_cmd = _dockerfile_from_e2b_template_payload(body)
        ...
        await template_handlers._finish_build_record(
            ...
            build_log="E2B SDK template build completed",
        )
    except HTTPException as ex:
        await template_handlers._finish_build_record(
            sandbox_manager,
...
async def get_template_file_upload_link(
    ...
    _ = (template_id, hash_, principal)
    return _error_response(400, "E2B SDK COPY/file-upload template builds are not implemented yet.")
