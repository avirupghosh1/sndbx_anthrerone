...
class SandboxRuntimeLostException(APIException):
    """Sandbox existed, but its backing runtime workload died."""

    def __init__(self, sandbox_id: str, message: str | None = None):
        detail = message or (
            "Previous sandbox container died after runtime-gateway restart; recreate the sandbox."
        )
        super().__init__(
            detail,
            status_code=410,
            details={"sandbox_id": sandbox_id, "reason": "runtime_lost"},
        )
