...
class _PostgresDatabase:
    ...
    def _acquire_postgres_advisory_lock(self, lock_name: str) -> bool:
    ...
    def acquire_advisory_lock(self, lock_name: str) -> bool:
        return self._acquire_postgres_advisory_lock(lock_name)
        ...
        with self._lock:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS observability_metric_samples (
                    ...
                    gateway_instance_id TEXT NOT NULL DEFAULT '',
                    warm_pool_key TEXT NOT NULL DEFAULT '',
            ...
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_events_sandbox
                ON observability_events(sandbox_id, timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_events_template
                ON observability_events(template_id, timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_events_gateway
                ON observability_events(gateway_instance_id, timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_events_category_action
                ON observability_events(category, action, timestamp DESC)
                """
            )
            cursor.execute(
                """
            ...
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_metric_samples_timestamp
                ON observability_metric_samples(timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_metric_samples_type
                ON observability_metric_samples(sample_type, timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_metric_samples_gateway
                ON observability_metric_samples(gateway_instance_id, timestamp DESC)
                """
            )
            cursor.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_observability_metric_samples_pool
                ON observability_metric_samples(warm_pool_key, timestamp DESC)
                """
            )
    ...
    def disable_warm_pool_segment(self, warm_pool_key: str, message: Optional[str] = None) -> bool:
        ...
        with self._lock:
            ...
            cursor.execute(
                ...
                SET desired_size = 0,
                    inflight_count = 0,
                    inflight_updated_at = NULL,
                    preferred_gateway_instance_id = NULL,
                    last_error = ?,
                    updated_at = ?
                WHERE warm_pool_key = ?
                """,
    ...
    def list_sandboxes_for_gateway(
        ...
        *,
        ...
        """List running sandboxes currently assigned to a runtime gateway."""
        gateway = (gateway_instance_id or "").strip()
        if not gateway:
            return []
        with self._lock:
            ...
            cursor.execute(
                ...
                ORDER BY is_warm_pool DESC, created_at DESC
                LIMIT ?
                """,
                (gateway, int(limit)),
            )
    ...
    @staticmethod
    def _observability_event_dict_from_row(cursor, row: tuple) -> Dict[str, Any]:
        names = [d[0] for d in cursor.description]
        src = dict(zip(names, row))
        try:
            metadata = json.loads(src.get("metadata_json") or "{}")
        except Exception:
            metadata = {}
        ...
        return {
            ...
            "gateway_instance_id": src.get("gateway_instance_id") or "",
            "template_id": src.get("template_id") or "",
            "sandbox_id": src.get("sandbox_id") or "",
            "message": src.get("message") or "",
            "metadata": metadata,
        }

    @staticmethod
    def _observability_metric_sample_dict_from_row(cursor, row: tuple) -> Dict[str, Any]:
        names = [d[0] for d in cursor.description]
        src = dict(zip(names, row))
        try:
            metrics = json.loads(src.get("metrics_json") or "{}")
        except Exception:
            metrics = {}
        ...
        return {
            ...
            "timestamp": src.get("timestamp"),
            "sample_type": src.get("sample_type") or "",
            "gateway_instance_id": src.get("gateway_instance_id") or "",
            "warm_pool_key": src.get("warm_pool_key") or "",
            "metrics": metrics,
        }
        ...
        *,
        ...
        with self._lock:
            ...
            cursor.execute(
                ...
                (event_id, timestamp, severity, category, action, entity_type, entity_id,
                 gateway_instance_id, template_id, sandbox_id, message, metadata_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    payload["event_id"],
    ...
    def list_observability_events(
        ...
        *,
        ...
        for column, value, lower in (
            ...
            ("gateway_instance_id", gateway_instance_id, False),
            ("template_id", template_id, False),
            ("sandbox_id", sandbox_id, False),
        ):
            raw = (value or "").strip()
            if not raw:
                continue
            where.append(f"{column} = ?")
            params.append(raw.lower() if lower else raw)
        if (since or "").strip():
        ...
        with self._lock:
            ...
            cursor.execute(sql, tuple(params))
            rows = cursor.fetchall()
            out = [self._observability_event_dict_from_row(cursor, row) for row in rows]
            conn.close()
        return out
    ...
    def record_observability_metric_sample(
        ...
        *,
        ...
        with self._lock:
            ...
            cursor.execute(
                """
                INSERT INTO observability_metric_samples
                (sample_id, timestamp, sample_type, gateway_instance_id, warm_pool_key, metrics_json)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    payload["sample_id"],
    ...
    def list_observability_metric_samples(
        ...
        *,
        ...
        if (sample_type or "").strip():
            ...
            params.append(sample_type.strip().lower())
        if (gateway_instance_id or "").strip():
            where.append("gateway_instance_id = ?")
            params.append(gateway_instance_id.strip())
        if (warm_pool_key or "").strip():
            where.append("warm_pool_key = ?")
            params.append(warm_pool_key.strip())
        if (since or "").strip():
            where.append("timestamp >= ?")
            params.append(since.strip())
        if (until or "").strip():
            where.append("timestamp <= ?")
            params.append(until.strip())
        sql = "SELECT * FROM observability_metric_samples"
        if where:
            sql += " WHERE " + " AND ".join(where)
        sql += " ORDER BY timestamp " + ("ASC" if ascending else "DESC") + " LIMIT ?"
        ...
        with self._lock:
            ...
            cursor.execute(sql, tuple(params))
            rows = cursor.fetchall()
            out = [self._observability_metric_sample_dict_from_row(cursor, row) for row in rows]
            conn.close()
        return out
    ...
    def purge_observability_before(self, cutoff_iso: str) -> Dict[str, int]:
        ...
        with self._lock:
            conn = self._connect()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM observability_events WHERE timestamp < ?", (cutoff,))
            event_count = int(cursor.rowcount or 0)
            cursor.execute("DELETE FROM observability_metric_samples WHERE timestamp < ?", (cutoff,))
            sample_count = int(cursor.rowcount or 0)
            conn.commit()
            conn.close()
        return {"events": event_count, "metric_samples": sample_count}
